...
class DLL_EXPORT NAbstractTableModel : public QAbstractItemModel,
                                      public EventWidget {
  Q_OBJECT
 ...
 public:
  ...
  void setDisplayNumRows(int numRows) {
    displayCount = numRows;
  }
...
protected:
  ...
  void fetchMore(const QModelIndex &parent) override {
    if (parent.isValid())
        return;
    int remainder = totalRows - rowsFetched;
    int itemsToFetch = qMin(displayCount, remainder);

    if (itemsToFetch <= 0)
        return;

    beginInsertRows(QModelIndex(), rowsFetched, rowsFetched + itemsToFetch - 1);

    rowsFetched += itemsToFetch;
